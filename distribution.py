...
def get_request_nodes(dev, node, request_update, request_update_id, request_update_result):
    ...
    """
    req_dict = {'request': dev.code_to_node.get(code2format_str(node['blockId'], node['index'],node['category'], node['code']+"_"+request_update)),
                'id': dev.code_to_node.get(code2format_str(node['blockId'], node['index'],node['category'], node['code']+"_"+request_update_id)),
                'result': dev.code_to_node.get(code2format_str(node['blockId'], node['index'],node['category'], node['code']+"_"+request_update_result))}
    return req_dict
...
class distribution_server(object):
    ...
    # recipe 改为并发向模组下载数据
    async def request_recipe_handle_gather(self, url, req, dev, module, write_recipe_id):
        """
        ...
        if datas is None:  # no response or response none
            ...
            for mr in datas['data']:
                tasks.append(self.process_recipe_data(mr, dev, module, all_success))
    ...
    async def process_recipe_data(self, mr, dev, module, all_success):
        """
        ...
        # parse json datas
        result = self.json_data_parse(mr, dev, module)
        parse_time = int(time.time() * 1000)
    ...
    def process_recipes(self, datas):
        # 分类映射字典（category -> uri）
        category_uri_map = {}
        for item in self.recipe_single_module:
        ...
        for item in self.recipe_request_data:
            category = item["module"]["category"]
            uri = item["uri"]
            if category in category_uri_map and category_uri_map[category] != uri:
                raise ValueError(f"Conflict URI for category {category}")
    ...
    async def request_recipe_handle_gather_plc(self, url, req, dev, module, write_recipe_id):
        """
        request recipe handle
        :param url: server url
        :param req: request nodes
        ...
        if datas is None:  # no response or response none
            ...
            grouped_data = self.process_recipes(datas)

            tasks = []
            for recipe_data in grouped_data:
                tasks.append(self.write_recipe_handle(grouped_data.get(recipe_data), dev, module, req))
            results = await asyncio.gather(*tasks)
            all_success = all(results)
    ...
    async def write_recipe_handle(self, arr, dev, module, req):
        ...
        for mr in arr:
            ...
            if mc_match := self.recipe_request_map.get(key):  # 针对MC做特殊处理
                mr["list"][0]["value"]["Basic"]["Id"] = 0
    ...
    async def request_recipe_handle(self, url, req, dev, module, write_recipe_id):
        """
        request recipe handle
        :param url: server url
        :param req: request nodes
        ...
        if datas is None:  # no response or response none
            ...
            print(f'{get_current_time()}: 配方开始向PLC下载...')
            log.info(f' 配方开始向PLC下载...')
            all_success = await self.write_recipe_handle(datas['data'], dev, module, req)
            # check all recipe write success or not
                ...
                if dev.connecting is True:
                    try:
                        ...
                        if req['request']["value"] is True and req['result']["value"] == 0:  #
                            ...
                            await self.request_recipe_handle(self.config['Server']['Basic']['recipe_req_url'], req, dev,
                                                             module, write_recipe_id)
                            # await self.request_recipe_handle_gather(self.config['Server']['Basic']['recipe_req_url'],
                            #                                         req, dev, module, write_recipe_id)  # 并发下发Recipe-单模组
                            # await self.request_recipe_handle_gather_plc(self.config['Server']['Basic']['recipe_req_url'],
