...
# driver logging init.
log = logging.getLogger('drv_tool')
...
#创建了一个时间滚动文件处理器（TimedRotatingFileHandler），用于将日志消息写入到文件中，并在指定的时间点滚动日志文件。这里设置为每天午夜（when='midnight'）滚动一次（interval=1），并保留30个备份文件（backupCount=30）。
time_rotating_file_handler = TimedRotatingFileHandler(filename=f'{log_dir}/drv_tool_', encoding='utf-8',
                                                      when='midnight', interval=1, backupCount=30)
